...
type TrailParty={code:string;name:string;memberName:string;role:"ORGANIZER"|"MEMBER";createdAt:string};
...
const partyKey="raahi_trail_party";
const regroupKey="raahi_regroup_point";
...
export default function RaahiApp({user}:{user:SignedInUser|null}){
 ...
 const [party,setParty]=useState<TrailParty|null>(null),[partyName,setPartyName]=useState("Triund trail party"),[memberName,setMemberName]=useState(user?.displayName??"Aastha"),[joinCode,setJoinCode]=useState("");
 ...
 useEffect(()=>{try{setReports(JSON.parse(localStorage.getItem(key)||"[]"));setParty(JSON.parse(localStorage.getItem(partyKey)||"null"));setRegroup(localStorage.getItem(regroupKey)||"Forest Bend shelter")}catch{}const invited=new URLSearchParams(window.location.search).get("party");if(invited)setJoinCode(invited.toUpperCase());readPack("TRIUND").then(setPack).catch(()=>undefined);if("serviceWorker" in navigator) navigator.serviceWorker.register("/sw.js").catch(()=>undefined)},[]);
 useEffect(()=>{localStorage.setItem(key,JSON.stringify(reports))},[reports]);
 useEffect(()=>{if(party)localStorage.setItem(partyKey,JSON.stringify(party));else localStorage.removeItem(partyKey)},[party]);
 useEffect(()=>{localStorage.setItem(regroupKey,regroup)},[regroup]);
 ...
 function createParty(){const name=memberName.trim();if(!name){setToast("Add your name before creating a party.");return}const code=crypto.randomUUID().replaceAll("-","").slice(0,6).toUpperCase();setParty({code,name:partyName.trim()||`${selectedTrek.name} party`,memberName:name,role:"ORGANIZER",createdAt:new Date().toISOString()});setToast(`Party ${code} created and saved on this device.`)}
 function joinParty(){const code=joinCode.trim().toUpperCase().replace(/[^A-Z0-9]/g,"");const name=memberName.trim();if(code.length<4||!name){setToast("Enter your name and a valid party code.");return}setParty({code,name:`${selectedTrek.name} party`,memberName:name,role:"MEMBER",createdAt:new Date().toISOString()});setJoinCode(code);setToast(`Joined party ${code}. Membership is saved offline on this device.`)}
 async function shareParty(){if(!party)return;const url=`${window.location.origin}${window.location.pathname}?party=${party.code}`;const text=`Join my ${party.name} in Raahi with code ${party.code}`;try{if(navigator.share){await navigator.share({title:"Join my Raahi Trail Party",text,url});return}await navigator.clipboard.writeText(`${text}\n${url}`);setToast("Invite link copied. Share it before the trek while everyone has network.")}catch(error){if((error as DOMException).name!=="AbortError")setToast(`Party code: ${party.code}`)}}
 function leaveParty(){setParty(null);setFriends(defaultFriends);setToast("Trail Party removed from this device.")}
 function scanParty(){if(!party){setToast("Create or join a party first.");return}setScanning(true);setTimeout(()=>{setFriends(v=>v.map(f=>f.id==="M03"?{...f,distance:86,lastSeen:"Just spotted · simulated",status:"NEARBY"}:f));setScanning(false);setToast("Demo scan complete. Native BLE detection remains final-app scope.")},900)}
 ...
 return <main className="app-shell">
  ...
  {view==="party"&&!party&&<section className="party-setup"><header><p className="eyebrow amber">TRAIL PARTY SETUP</p><h1>Connect before the signal ends.</h1><p>Create one party and share its invite code while everyone has network. Each member saves the code and regroup point locally for the trek.</p></header><div className="party-choice-grid"><article className="party-choice"><span className="party-step">01</span><h2>Create a party</h2><label className="field-label">Your name<input value={memberName} onChange={e=>setMemberName(e.target.value)} placeholder="Your name"/></label><label className="field-label">Party name<input value={partyName} onChange={e=>setPartyName(e.target.value)} placeholder="Triund trail party"/></label><button className="primary" onClick={createParty}>Create invite code</button></article><article className="party-choice"><span className="party-step">02</span><h2>Join a party</h2><label className="field-label">Your name<input value={memberName} onChange={e=>setMemberName(e.target.value)} placeholder="Your name"/></label><label className="field-label">Invite code<input value={joinCode} onChange={e=>setJoinCode(e.target.value.toUpperCase())} placeholder="ABC123" maxLength={10}/></label><button className="primary" onClick={joinParty}>Join and save offline</button></article></div><p className="prototype-note">Web-demo pairing is stored only in this browser. The final native app will verify membership through the backend and exchange rotating encrypted BLE identifiers offline.</p></section>}
  {view==="party"&&party&&<section className="panel-grid party-layout"><article className="form-card"><div className="party-session"><div><p className="eyebrow amber">{party.role==="ORGANIZER"?"PARTY ORGANIZER":"PARTY MEMBER"}</p><h1>{party.name}</h1><p>Signed in locally as <strong>{party.memberName}</strong>. Party membership and the regroup point remain available on this device offline.</p></div><div className="party-code"><small>INVITE CODE</small><strong>{party.code}</strong><span>Saved offline</span></div></div><div className="party-actions"><button className="primary" onClick={shareParty}>Share invite</button><button className="secondary" onClick={leaveParty}>Leave party</button></div><div className="party-radar" aria-label="Simulated offline friend proximity view"><span className="radar-ring ring-one"/><span className="radar-ring ring-two"/><span className="radar-you">You</span>{friends.map((f,i)=><span key={f.id} className={"radar-friend friend-"+i}>{f.name[0]}</span>)}</div><button className="primary" onClick={scanParty} disabled={scanning}>{scanning?"Running simulated proximity scan…":"Run demo proximity scan"}</button><p className="prototype-note">The invite and offline membership flow are functional. Nearby distances are simulated in this web demo; genuine background detection is reserved for the native BLE app.</p></article><aside className="queue-card"><p className="eyebrow">DEMO GROUP · {friends.length+1}</p><ul className="friend-list"><li><span><strong>{party.memberName} (you)</strong><small>This device · party {party.code}</small></span><em className="nearby">Ready</em></li>{friends.map(f=><li key={f.id}><span><strong>{f.name}</strong><small>{f.lastSeen}</small></span><em className={f.status.toLowerCase()}>{f.distance===null?"Out of range":f.distance<25?"Very close":f.distance<60?"Nearby":"Weak signal"}</em></li>)}</ul><label className="field-label">Regroup point<select value={regroup} onChange={e=>setRegroup(e.target.value)}><option>Forest Bend shelter</option><option>Magic View checkpoint</option><option>Gallu trailhead</option></select></label><div className="regroup-card"><span>Agreed regroup point</span><strong>{regroup}</strong><small>Saved locally for offline access on this device</small></div></aside></section>}
